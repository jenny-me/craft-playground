    ...
    if (settings.arrowButtons || settings.animate) {
      ...
      if (settings.animate) {
        ...
        if (settings.startAnimated) {
          ctrls.innerHTML +=
            "<li>" +
            '<button data-stop="true"><span class="visuallyhidden">Stop Animation </span></button>' +
            "</li>";
        } else {
          ctrls.innerHTML +=
            "<li>" +
            '<button data-start="true"><span class="visuallyhidden">Start Animation </span></button>' +
            "</li>";
  ...
  function setSlides(new_current, focus, transition) {
    ...
    if (settings.slidenav) {
      ...
      for (var j = buttons.length - 1; j >= 0; j--) {
        buttons[j].className = "";
        buttons[j].innerHTML =
          '<span class="visuallyhidden">Slide</span> ' + (j + 1);
      }
